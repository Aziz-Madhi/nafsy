To rate limit your app’s voice feature using tokens with the Convex Rate Limiter component, you’ll want to:

1. **Define a per-user token bucket rate limit** for the voice feature.
2. **Check and consume tokens** before or after each OpenAI API call, depending on whether you can estimate or know the actual token usage.
3. **Update the rate limiter** with the number of tokens used, so users are prevented from exceeding their quota.

### 1. Configure the Rate Limiter

First, set up a per-user token bucket rate limit. For example, to allow each user 10,000 tokens per hour:
...
const rateLimiter = new RateLimiter(components.rateLimiter, {
  voiceTokens: { kind: 'token bucket', rate: 10000, period: HOUR },
});
```

You can adjust `rate` and `period` as needed for your use case. This setup ensures each user (when you pass their user ID as a key) can use up to 10,000 tokens per hour for the voice feature.  
[See: Define your rate limits](https://www.convex.dev/components/rate-limiter#define-your-rate-limits)

### 2. Check and Consume Tokens

**Before making the OpenAI API call:**  
If you can estimate the number of tokens the request will use, check the rate limit before sending the request:

```js
const estimatedTokens = ...; // Estimate based on input length, etc.
const status = await rateLimiter.limit(ctx, "voiceTokens", { key: userId, count: estimatedTokens });
if (!status.ok) {
  // Inform the user to wait until status.retryAfter
}
```

[See: Consume a custom count](https://www.convex.dev/components/rate-limiter#usage)

**After receiving the OpenAI response:**  
If you only know the actual token usage after the API call (which is typical), consume the tokens then:

```js
const actualTokens = openaiResponse.data.usage.total_tokens;
await rateLimiter.limit(ctx, 'voiceTokens', {
  key: userId,
  count: actualTokens,
});
```

[See: Post-generation Usage Tracking](https://docs.convex.dev/agents/rate-limiting#how-it-works)

### 3. Handling OpenAI Token Usage

- For non-streaming OpenAI API calls, the response includes a `usage` field with the number of tokens used.
- For streaming, you may need to estimate tokens before the call, as the actual count may not be available until the stream ends. See [Token Estimation](https://docs.convex.dev/agents/rate-limiting#token-estimation) for a simple estimation approach.

### 4. Example Flow

```js
// 1. (Optional) Pre-check if you want to estimate and block early
const estimatedTokens = estimateTokens(...);
const preCheck = await rateLimiter.check(ctx, "voiceTokens", { key: userId, count: estimatedTokens });
if (!preCheck.ok) {
  // Block the request, inform user to wait
}

// 2. Make the OpenAI API call
const openaiResponse = await openai.createCompletion(...);

// 3. After the call, consume the actual tokens used
const actualTokens = openaiResponse.data.usage.total_tokens;
await rateLimiter.limit(ctx, "voiceTokens", { key: userId, count: actualTokens });
```

[See: How It Works](https://docs.convex.dev/agents/rate-limiting#how-it-works)

---

This approach ensures each user is rate limited by the number of tokens they use for the voice feature, leveraging Convex’s transactional guarantees and the Rate Limiter component. If the user exceeds their quota, you can inform them how long to wait before trying again, using the `retryAfter` value returned by the limiter.

To measure API usage through OpenAI voice model tokens and enable Convex to rate limit users precisely, you should:

1. **Extract the token usage from the OpenAI API response.**
   - For non-streaming OpenAI completions, the response includes a `usage` field (e.g., `openaiResponse.data.usage.total_tokens`) that tells you exactly how many tokens were used for the request.
   - For streaming completions, OpenAI’s API currently does **not** provide token usage in the response, so you’ll need to estimate tokens before the call or use heuristics after the fact. See [Token Estimation](https://docs.convex.dev/agents/rate-limiting#token-estimation) for an example estimation function.

2. **Consume tokens in the Convex Rate Limiter after the API call.**
   - Once you have the actual token count, call the rate limiter to consume that number of tokens for the user. This ensures you are rate limiting based on the exact usage, not an estimate.

   Example (after receiving the OpenAI response):
   ...
   This will decrement the user’s available tokens and, if they exceed their quota, block further requests until their allowance resets. If the limit is exceeded, you can use the `retryAfter` value to inform the user when they can try again.  
   [See: How It Works](https://docs.convex.dev/agents/rate-limiting#how-it-works), [Consume a custom count](https://www.convex.dev/components/rate-limiter#usage)

3. **(Optional) Track usage for analytics or billing.**
   - You can also store each usage event in a Convex table for later analysis or invoicing, as shown in the [Usage Tracking example](https://docs.convex.dev/agents/usage-tracking).

**Summary:**

- For non-streaming: use the `usage` field from OpenAI’s response to get the exact token count and pass it to the Convex rate limiter.
- For streaming: estimate tokens before the call, or use heuristics, since the exact count may not be available.

This approach ensures Convex always knows the precise token usage and can enforce rate limits at the exact point of use.
...
When [establishing a peer connection](/docs/guides/realtime-webrtc) you fetch an ephemeral token secret from the application backend, then pass it to the WebRTC client (e.g. web frontend or mobile app), then fetch and receive an SDP response from the Realtime API to configure the connection. If you used the sample code from the WebRTC guide, that looks something like this:
...
2.  OpenAI sends a webhook to your application’s backend webhook URL, notifying your app of the state of the session. The webook will look something like:
