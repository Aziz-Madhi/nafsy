              ...
              >
                ...
                <View className="flex-row items-center justify-between mb-6">
                  ...
                  <View className="items-center">
                    <View className="relative w-20 h-20 items-center justify-center">
                      ...
                      <View
                        ...
                        style={{
                          ...
                          borderTopColor:
                            (userStats?.completionsThisWeek || 0) >= 2
                              ? '#5A4A3A'
                              : 'transparent',
                          borderRightColor:
                            (userStats?.completionsThisWeek || 0) >= 4
                              ? '#5A4A3A'
                              : 'transparent',
                          borderBottomColor:
                            (userStats?.completionsThisWeek || 0) >= 6
                              ? '#5A4A3A'
                              : 'transparent',
                          borderLeftColor:
                            (userStats?.completionsThisWeek || 0) >= 1
                              ? '#5A4A3A'
                              : 'transparent',
                        }}
                      />
                    ...
                    <Text
                      variant="caption1"
                      className="text-[#5A4A3A]/80 mt-2 text-center font-medium"
                    >
                      This Week
                    </Text>
                  </View>
                  ...
                  <View className="flex-1 ml-6">
                    ...
                    <View className="flex-row items-center mb-3">
                      <View className="flex-row items-center">
                        {Array.from({
                          length: Math.min(userStats?.currentStreak || 0, 5),
                        }).map((_, i) => (
                          <Text key={i} className="text-base mr-1">
                            🔥
                          </Text>
                        ))}
                ...
                <View className="mb-4">
                  <Text
                    variant="subhead"
                    className="text-[#5A4A3A]/80 mb-3 font-medium"
                  >
                    Weekly Progress
                  </Text>
                  <View className="flex-row justify-between items-center">
                    {['M', 'T', 'W', 'T', 'F', 'S', 'S'].map((day, index) => {
                      const isCompleted =
                        index < (userStats?.completionsThisWeek || 0);
                      return (
                  ...
                  <View className="flex-row items-center">
                    ...
                    <View>
                      ...
                      <Text variant="caption1" className="text-[#5A4A3A]/60">
                        Most practiced •{' '}
                        {Math.round(userStats?.averageSessionDuration || 5)}min
                        avg
                      </Text>
                    </View>
                  </View>
